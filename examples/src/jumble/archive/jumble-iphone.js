...
function loadArchivePuzzle(datestr) {
   /*   var archivedate = new Date();
   archivedate.setYear(parseInt("20" + datestr.substring(0,2)));
   archivedate.setMonth(parseInt(datestr.substring(2,4)));
   archivedate.setDate(parseInt(datestr.substring(4)));
...
function init() {
   ...
   var suffix = document.getElementById("dateinput").value;

   try {
      loadArchivePuzzle(suffix);
   } catch (e) {
      if (DEBUG) setStatus("error: " + e);
      error();
   }
