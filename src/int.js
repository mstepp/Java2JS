...
// cont expects a {div:int, mod:int} value
Integer.prototype.divmodCPS = function(other, cont, exc) {
   var that = this;
   return function() {
      function cont3(div, recurse) {
         var result = {div:div.add(recurse.div), mod:recurse.mod};
         return cont(result);
      }
      ...
      function cont2() {
         if (other.equals(Integer.MIN_VALUE)) {
            if (that.equals(other)) {
               ...
               return {div:Integer.ZERO, mod:that};
            }
         }
         
         if (that.isNegative()) {
            if (other.lt(that) ||
                (!other.isNegative() && other.negate().lt(that))) {
               return {div:Integer.ZERO, mod:that};
            }
         } else {
            if (other.gt(that) ||
                (other.isNegative() && other.negate().gt(that))) {
               return {div:Integer.ZERO, mod:that};
            }
         }
         ...
         return that.subtract(prod).divmodCPS(other, 
                                              function(recurse) {return cont3(div, recurse);},
                                              exc);
      }
      return Util.assertWithException(!other.equals(Integer.ZERO), "java.lang.ArithmeticException", cont2, exc);
   };
};
